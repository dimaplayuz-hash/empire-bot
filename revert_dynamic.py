...
# Revert start_command to ask for phone directly
...
# Remove API_ID/API_HASH handlers
old_handlers = '''# API_ID/API_HASH handlers
...
async def handle_login_api_hash(client, message, user_id, text):
    ...
    await message.reply_text(
        ...
        f"Agar oldin session yaratgan bo'lsangiz, .session faylni yuborishingiz mumkin."
    )
    return True
...
new_handlers = '''# Telefon raqam formatini tekshirish'''

content = content.replace(old_handlers, new_handlers)

# Revert process_messages
old_process = '''    # Login flow - API_ID kiritilganda
...
new_process = '''    # Login flow - session fayl yuborilganda
    if state == "login_upload":
        if message.document:
            await handle_login_upload(client, message, user_id)
        else:
...
# Revert handle_login_phone to use config API_ID/API_HASH
...
# Revert handle_login_code
old_code = '''    # API_ID/API_HASH olish
    if user_id not in login_data or "api_id" not in login_data[user_id]:
        await message.reply_text("❌ API_ID/API_HASH topilmadi. Qaytadan boshlang.")
        return False
...
new_code = '''    # Clientni qaytadan yaratish va saqlash
    user_client = Client(
        session_name,
        api_id=config["API_ID"],
        api_hash=config["API_HASH"],
...
# Revert handle_login_password
old_password = '''    # API_ID/API_HASH olish
    if user_id not in login_data or "api_id" not in login_data[user_id]:
        await message.reply_text("❌ API_ID/API_HASH topilmadi. Qaytadan boshlang.")
        return False
...
new_password = '''    # Clientni qaytadan yaratish va saqlash
    user_client = Client(
        session_name,
        api_id=config["API_ID"],
        api_hash=config["API_HASH"],
...
# Revert handle_login_upload
old_upload = '''            # API_ID/API_HASH olish (agar session fayl yuborilsa, config dan olinadi)
            api_id = config["API_ID"]
            api_hash = config["API_HASH"]
...
new_upload = '''            # Client yaratish va tekshirish
            session_name = f"sessions/user_{user_id}"
            user_client = Client(
                session_name,
                api_id=config["API_ID"],
...
# Revert cancel_command
old_cancel = '''    if state in ["login_api_id", "login_api_hash", "login_phone", "login_code", "login_password", "login_upload"]:'''

new_cancel = '''    if state in ["login_phone", "login_code", "login_password", "login_upload"]:'''
...
print('Reverted dynamic API changes - back to simple phone/code/2FA login')
